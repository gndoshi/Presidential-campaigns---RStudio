# Presidential-campaigns---RStudio
...
Yes, this graph is not clear at all with names over-lapping and not giving us enough information with how other candidates are doing. 
Also, we can see that there are some negative receipt amounts. This really looks like there were some residents who initiated a refund and got their money back.
...
Now, let's zoom into this graph by setting an appropriate y-axis liit as 800 so we can see what's going on down there. 
```{r echo=FALSE, message=FALSE, warning=FALSE}
ggplot(aes(x = contb_receipt_amt), data = fincontnc) + geom_point(stat = 'bin',aes(fill = ..count.., color = party), binwidth = 10) + 
...
I was surprised to see the amounts contributed to Republican candidates. They were large sums and help bridge a lot of the gap between themselves and he Democrats. The latter had a huge number of supporters (contributions) though and it was easy to state that Hillary Clinton and Bernie Sanders were the 2 most popular candidates in the state of North Carolina. 
